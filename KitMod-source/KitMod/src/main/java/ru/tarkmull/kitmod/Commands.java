...
public final class Commands implements CommandExecutor, TabCompleter {
    ...
    private boolean kitmod(CommandSender sender, String[] args) {
        ...
        if (args.length >= 1 && args[0].equalsIgnoreCase("check")) {
            sender.sendMessage(Msg.c("&8--- &cДиагностика KitMod &8---"));
            sender.sendMessage(Msg.c("&7Версия сервера: &f" + Bukkit.getVersion()));
            boolean supported = ItemRegistry.itemModelSupported();
            sender.sendMessage(Msg.c("&7Компонент item_model: "
                    + (supported ? "&aподдерживается" : "&cНЕ поддерживается")));
            sender.sendMessage(Msg.c("&7Реально применён: "
                    + (ItemRegistry.itemModelWorks() ? "&aда" : "&eещё не применялся")));
            sender.sendMessage(Msg.c("&7Запасной custom_model_data: &aвключён (1–17)"));
            if (!supported) {
                sender.sendMessage(Msg.c("&eМодели будут работать через custom_model_data — "
                        + "это нормально, ресурспак умеет оба способа."));
            }
            sender.sendMessage(Msg.c("&7Ресурспак в server.properties: &f"
                    + (Bukkit.getResourcePack() == null || Bukkit.getResourcePack().isEmpty()
                    ? "&cне указан" : "&aуказан")));
            return true;
        }
        ...
        sender.sendMessage(Msg.c("&f/kitmod reload|list|check"));
        return true;
    }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (cmd.equals("kitmod")) {
            if (args.length == 1) {
                for (String s : List.of("reload", "list", "check")) {
