...
public final class ItemRegistry {
    ...
    /**
     * Запасной путь на случай, если сервер не умеет компонент item_model:
     * ресурспак ловит эти числа через custom_model_data.
     * Значения ДОЛЖНЫ совпадать с ресурспаком (assets/minecraft/items/*.json).
     */
    public static final java.util.Map<String, Integer> MODEL_DATA = java.util.Map.ofEntries(
            java.util.Map.entry("pizza", 1),
            java.util.Map.entry("pizza_sword", 2),
            java.util.Map.entry("leviathan", 3),
            java.util.Map.entry("vampire_scythe", 4),
            java.util.Map.entry("legendary_sword", 5),
            java.util.Map.entry("lava_crystal", 6),
            java.util.Map.entry("reinforced_string", 7),
            java.util.Map.entry("dragon_ingot", 8),
            java.util.Map.entry("ender_ingot", 9),
            java.util.Map.entry("blood_ingot", 10),
            java.util.Map.entry("lava_ingot", 11),
            java.util.Map.entry("steel", 12),
            java.util.Map.entry("ruby_diamond", 13),
            java.util.Map.entry("rare_glove", 14),
            java.util.Map.entry("master_redstone", 15),
            java.util.Map.entry("lava_mold", 16),
            java.util.Map.entry("lava_mold_filled", 17));

    private final KitModPlugin plugin;
    private final NamespacedKey idKey;
    private static Method setItemModel;
    private static boolean modelWarned = false;
    /** Удалось ли выставить компонент item_model хотя бы раз. */
    private static boolean itemModelWorks = false;

    public static boolean itemModelWorks() {
        return itemModelWorks;
    }
    ...
    public ItemStack create(String id, int amount) {
        ...
        // 3D-модель из ресурспака: основной путь — компонент item_model
        applyModel(meta, new NamespacedKey(NAMESPACE, id));

        // ...и запасной — custom_model_data (работает на любой 1.21.4)
        Integer cmd = s != null && s.contains("custom-model-data")
                ? s.getInt("custom-model-data")
                : MODEL_DATA.get(id);
        if (cmd != null && cmd > 0) {
